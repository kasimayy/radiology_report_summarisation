''' Seq2Seq Report Summarisation

Dataset: Openi Chest X-ray raw reports + MeSH term annotations

Preprocessing steps: For text reports, lower-casing, punctuation removal, negated sentence removal, removal of words appearing <5 times, tokenization, padding/cropping, vectorising into list of word ids. For MeSH annotations, lower-casing, removal of words appearing <n times, one-hot encoding.
...
print('Inputs shape: ', one_hot_reports_train.shape, one_hot_mesh_train.shape, one_hot_mesh_shifted_train.shape)
...
# bleu
def evaluate_model_bleu(model, df, report_vocab_length):
...
train_actual, train_predicted = evaluate_model_bleu(old_experiment, train_df, report_vocab_length)
val_actual, val_predicted = evaluate_model_bleu(old_experiment, val_df, report_vocab_length)
